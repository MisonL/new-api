...
} from "../classic/src/helpers/authRedirect.js";
import { setUserData } from "../classic/src/helpers/data.js";
...
test("normalizeAuthRedirectTarget only allows local app paths", () => {
  ...
  assert.equal(normalizeAuthRedirectTarget("javascript:alert(1)"), "");
  assert.equal(
    normalizeAuthRedirectTarget(
  ...
  assert.equal(normalizeAuthRedirectTarget("vbscript:msgbox(1)"), "");
  assert.equal(normalizeAuthRedirectTarget("jav%61script:alert(1)"), "");
  assert.equal(normalizeAuthRedirectTarget("/login"), "");
  assert.equal(normalizeAuthRedirectTarget("/register"), "");
  assert.equal(normalizeAuthRedirectTarget("/safe/path"), "/safe/path");
});
