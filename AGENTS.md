...
- 前端测试放在 `web/default` 对应模块同层的 `*.test.js` / `*.test.mjs` 文件，或后续明确约定的前端测试目录。
- 不得为测试新增隐藏开关、特殊成功路径或仅测试可见的业务分支。

### 4.2 验证基线

- 代码必须可测试，优先使用可重复的自动化验证。
- 后端改动优先运行最小相关 `go test`；前端改动优先在 `web/default` 下运行最小相关 `bun run lint` 与 `bun run build`，必要时补 `bun run i18n:sync` 和 `bun run typecheck`。
...
理解当前阶段代码真相时，优先查阅 `docs/reviews/` 和 `docs/architecture/` 的现有文件。

- 当前仓库暂无稳定的 `CR-STAGE-ALIGNMENT-*` 或 `CR-ISSUES-CURRENT-STATE-*` 总览文件；不要引用不存在的阶段总览占位文档。
- 优先按主题查找最新 `CR-*.md`，并确认文件在仓库内实际存在。
- 当前高价值索引：
  - 当前分支若存在 `docs/reviews/CR-CLI-HEADER-PROFILE-REALITY-*.md`，优先查看最新一份
...
- 安装依赖：`cd web/default && bun install`
- 开发：`cd web/default && bun run dev`
- 构建：`cd web/default && bun run build`
- i18n 工具：`cd web/default && bun run i18n:*`
...
- 仅有会话 Cookie 不够
- 必须额外带上 `New-Api-User` 请求头，值为当前登录用户 ID
...
- `SQLITE_PATH` 必须指向桌面应用数据目录下的 `data/new-api.db`
...
使用最新代码重建完全隔离开发环境时，默认使用 `scripts/build-docker-local.sh new-api-local:dev` 构建镜像，再只重建 `new-api` 服务：
...
验证 `web/default` 前端时，如果 `3001` 容器入口仍由系统配置加载 classic 主题，必须另起 `web/default` 开发服务器代理到隔离后端，而不是用 `3001` 下的 classic 页面替代验证：
...
- 前端改动后，至少重新执行一次 `cd web/default && bun run build` 确认可构建。
...
- CLI 真实请求头、Header Profile 与 `pass_headers` 透传边界，优先查当前分支最新的 `docs/reviews/CR-CLI-HEADER-PROFILE-REALITY-*.md`；若不存在，再看 `docs/reviews/CR-HEADER-PROFILE-STRATEGY-2026-04-21.md`。
- 隔离开发环境重建、`3001` 后端与 `web/default` 前端验证口径，优先查 `docs/reviews/CR-DEV-ISOLATED-VERIFY-2026-05-07.md`。
